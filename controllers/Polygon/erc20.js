...
var web3 = new Web3(
  new Web3.providers.HttpProvider(config.daemons.polygon.provider)
);
...
router.post("/transfer", async function (request, response) {
  try {
    ...
    if (weiBalance < txFee) {
      return response.status(400).json({
        msg: `Insufficient balance. transaction fee is ${weiBalance} ETH.`,
      });
    }
    console.log(9);
    ...
    let transactionObject = {
      ...
      chainId: config.daemons.polygon.provider.chainId,
    };
    console.log(11);
...
router.get(
  ...
  async (req, response) => {
    ...
    console.log('here', walletAddress, contractAddress);

    if (
      !ethereum_address.isAddress(walletAddress) ||
      !ethereum_address.isAddress(contractAddress)
    ) {
      return response.status(400).json({
        msg: "Invalid addresses",
